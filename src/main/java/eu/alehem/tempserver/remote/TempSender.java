...
public class TempSender implements Runnable {
    ...
    public void run() {
        ...
        List<TemperatureMeasurement> temperatureMeasurements = temperatures.stream()
                .map(t -> new TemperatureMeasurement(idGenerator.nextInt(10000), t.getMeasurementTimeServerFormat(), t.getTemperature()))
    ...
    private ServerTemperatureResponse sendToServer(String jsonData) throws ServerCommsFailedException {
        try {
            ...
            StringEntity entity = new StringEntity("data="+jsonData, ContentType.APPLICATION_FORM_URLENCODED);
            httpPost.setEntity(entity);
            HttpResponse response = client.execute(httpPost);

            Reader reader = new InputStreamReader(response.getEntity().getContent(), "UTF-8");
            ServerTemperatureResponse result  = new Gson().fromJson(reader, ServerTemperatureResponse.class);
            if (result == null) {
